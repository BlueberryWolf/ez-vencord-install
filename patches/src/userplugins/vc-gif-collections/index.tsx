...
import { Category, Collection, Props } from "./types";
	...
	renderContent(instance) {
		if (instance.props.query.startsWith(GIF_COLLECTION_PREFIX)) {
			...
			instance.props.resultItems = collection.gifs.map(g => ({
				id: g.id,
				format: getFormat(GetUpdatedGifCdnUrl(g.src, favoritedGifs.favorites)),
				src: GetUpdatedGifCdnUrl(g.src, favoritedGifs.favorites),
				url: GetUpdatedGifProxyCdnUrl(g.url, favoritedGifs.favorites),
				width: g.width,
				height: g.height
			})).reverse();
		}
